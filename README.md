...
**English TL;DR:** A Windows UIA watcher that auto-clicks the native "Allow remote debugging?" consent dialog in Edge/Chrome. Idle CPU ≈ 0.6%, worst-case reaction ~10s, typical ~2s. Runs hidden at logon via Task Scheduler.
...
**为什么不能常规自动化？** 弹窗是浏览器原生 UI，不在任何网页 DOM 里；而且它授予的正是 CDP 调试权限 —— 自动化点击本身需要先有这个权限（鸡生蛋）。组策略 `RemoteDebuggingAllowed` 只能整体开/关，没有"自动同意"开关。
...
>
> 另：PS 5.1 下 UIA 事件回调（脚本块从线程池线程投递）实测不可靠，所以 v3 保留了 10s 兜底扫描作为主要捕获路径。
